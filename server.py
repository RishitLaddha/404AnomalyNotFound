...
def get_capabilities():
    """
    Return MCP-style tool capabilities so an MCP client can
    discover what this server supports.
    """
    tools = [
        {
            "name": "kali_tools.nmap_scan",
            "description": "Execute an Nmap scan against a target.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "IP address or hostname to scan"},
                    "scan_type": {"type": "string", "description": "Nmap scan type flags", "default": "-sCV"},
                    "ports": {"type": "string", "description": "Comma-separated list of ports or ranges"},
        ...
        {
            "name": "kali_tools.gobuster_scan",
            "description": "Run Gobuster for directory/DNS/fuzz/vhost enumeration.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "url": {"type": "string", "description": "Target URL"},
                    "mode": {"type": "string", "description": "Gobuster mode: dir, dns, fuzz, vhost", "default": "dir"},
        ...
        },
        {
            ...
            "input_schema": {
                ...
                "properties": {
                    "url": {"type": "string", "description": "Target URL"},
                    "wordlist": {"type": "string", "description": "Path to wordlist file", "default": "/usr/share/wordlists/dirb/common.txt"},
                    "additional_args": {"type": "string", "description": "Additional Dirb arguments"},
                },
                "required": ["url"],
            },
        },
        {
            "name": "kali_tools.nikto_scan",
        ...
        {
            "name": "kali_tools.metasploit_run",
            "description": "Run a Metasploit module with given options.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "module": {"type": "string", "description": "Metasploit module path"},
                    "options": {"type": "object", "description": "Key/value module options"},
                },
                "required": ["module"],
            },
        },
        {
            "name": "kali_tools.hydra_attack",
            "description": "Run Hydra for password brute-forcing.",
            "input_schema": {
                "type": "object",
                "properties": {
                    ...
                    "username_file": {"type": "string", "description": "Path to username file"},
                    "password": {"type": "string", "description": "Single password"},
                    "password_file": {"type": "string", "description": "Path to password file"},
        ...
        {
            "name": "kali_tools.john_crack",
            "description": "Run John the Ripper password cracker.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "hash_file": {"type": "string", "description": "Path to file containing hashes"},
                    "wordlist": {"type": "string", "description": "Path to wordlist file", "default": "/usr/share/wordlists/rockyou.txt"},
                    "format": {"type": "string", "description": "Hash format type"},
                    "additional_args": {"type": "string", "description": "Additional John arguments"},
                },
                "required": ["hash_file"],
            },
        },
        {
            "name": "kali_tools.wpscan_analyze",
            "description": "Run WPScan against a WordPress site.",
            "input_schema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "kali_tools.enum4linux_scan",
            "description": "Run enum4linux for Windows/Samba enumeration.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "Target IP or hostname"},
                    "additional_args": {"type": "string", "description": "Additional enum4linux arguments", "default": "-a"},
                },
                "required": ["target"],
            },
        },
        {
            "name": "kali_tools.server_health",
            "description": "Check health of the Kali Tools API server.",
            "input_schema": {
                "type": "object",
                "properties": {},
                "required": [],
        ...
        {
            "name": "kali_tools.execute_command",
            "description": "Execute an arbitrary shell command on the Kali server.",
            "input_schema": {
                "type": "object",
                "properties": {
...
def execute_tool(tool_name):
    """
    Direct tool execution entry point for MCP clients.

    The MCP client should POST JSON arguments matching the
    corresponding /api/tools/* endpoints (or /api/command for
    execute_command). We simply dispatch to the existing Flask
    handlers so core behaviour stays in one place.
    """
    try:
        ...
        if tool_name == "metasploit_run":
            return metasploit()
        if tool_name == "hydra_attack":
            return hydra()
        if tool_name == "john_crack":
            return john()
        if tool_name == "wpscan_analyze":
            return wpscan()
        if tool_name == "enum4linux_scan":
            return enum4linux()
        ...
        logger.error(traceback.format_exc())
        return jsonify({"error": f"Server error executing tool {tool_name}: {str(e)}"}), 500
